...
class DataReader(Worker):
	def Init(self):
		...
		if not 'data_root' in self.__dict__.keys():
			self.data_root=default_data_root # execution folder
		self.dates={}
	...
	def Execute(self):
		...
		# since this could be in a completely separate process, 
		# made this process aware, which is where the 
			...
			with self.stats_sync:
				for origin in self.stats_cache:
					if today_ts in self.stats_cache[origin]:
						self.logger.debug("Updating daily statistics for {0} {1}...".format(origin,dt.fromtimestamp(today_ts,tzone.utc).isoformat()))
...
#			num_days=int(np.ceil((start_hour_dt-end_hour_dt).total_seconds()/(3600*24)))
			num_days=int(start_time_utc.timestamp()/(24*3600))-int(end_hour_dt.timestamp()/(24*3600)) + 1	
			if hourly:
...
#				logging.getLogger(__name__).debug("{0} Get data (cache)".format(self))
		...
		logging.getLogger(__name__).debug("XXX t,r: {0} {1}".format(t,r))
		logging.getLogger(__name__).debug("YYY oldest_ts, newest_ts: {0} {1}".format(oldest_ts,newest_ts))
	...
	def UpdateStats(self,origin=None,date_ts=None):
		if origin==None and date_ts==None:
			...
			c_dt=dt.fromtimestamp(date_ts,tzone.utc)
	...
	def CalculateStats(self,data,date_ts): 
		ret={}
		date=dt.fromtimestamp(date_ts,tzone.utc)
		ret['date']=dt.fromtimestamp(date_ts,tzone.utc).date().isoformat()
		tmin=date_ts
